  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {trigger ? (
        ...
        <DialogTrigger asChild>
          <Button 
            ...
            className="border-white/20 text-white hover:bg-white/10 transition-colors duration-300 focus:outline-hidden focus:ring-0"
          >
            + Nytt steg
          </Button>
        </DialogTrigger>
      ...
      <DialogContentNoFocus className="bg-card-parium border-white/20 sm:max-w-lg max-h-[calc(100dvh-40px)] grid-rows-[auto_minmax(0,1fr)_auto] !gap-0 overflow-hidden !p-0" hideClose>
        ...
        <div className="min-h-0 space-y-4 overflow-y-auto overscroll-contain px-6 py-3">
            ...
            <div className="grid grid-cols-10 gap-1 p-2 bg-white/5 rounded-lg">
              {JOB_STAGE_ICONS.map(({ name, Icon, label: iconLabel }) => (
                <button
                  ...
                  onMouseDown={(e) => e.currentTarget.blur()}
                  onMouseUp={(e) => e.currentTarget.blur()}
                  className={`p-2 rounded-full hover:bg-white/20 transition-colors duration-300 focus:outline-hidden focus:ring-0 ${
