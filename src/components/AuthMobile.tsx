              ...
              <CardContent className={cn("p-4 md:p-6", isLogin && (showResetPassword || resetPasswordSent) && "pb-24")}>
                 <Tabs value={isLogin ? 'login' : 'signup'} onValueChange={handleTabChange}>
                  ...
                  <div className="relative">
                    ...
                    <form key="login-form" ref={loginFormRef} onSubmit={handleSubmit} onKeyDown={(e) => { const tag = (e.target as HTMLElement)?.tagName; if (e.key === 'Enter' && !e.nativeEvent.isComposing && tag !== 'A' && tag !== 'TEXTAREA' && !loading) { e.preventDefault(); e.currentTarget.requestSubmit(); } }} className="space-y-3 md:space-y-4">
