      ...
      <Tooltip open={isShowingPreview || undefined}>
        <TooltipTrigger asChild>
          <button
            ...
            disabled={disabled}
            className={cn(
              "flex-shrink-0 flex items-center justify-center rounded-xl transition-all duration-150 caret-transparent",
              large ? "w-11 h-11" : compact ? "p-2" : "w-8 h-8",
          ...
          >
            <Icon className={cn(large ? "h-5 w-5" : compact ? "h-5 w-5" : "h-4 w-4", "text-pure-white")} />
          </button>
        </TooltipTrigger>
