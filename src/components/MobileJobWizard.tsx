...
const MobileJobWizard = ({
    ...
    if (open) {
      ...
      if (existingJob) {
        ...
        const loadedFormData: JobFormData = {
          ...
          start_date: effectiveStartDate((existingJob as any).start_date) || '',
  ...
  const resolveEditableImageUrl = async (source: string): Promise<string | null> => {
  ...
  const openImageEditor = async () => {
    try {
      // Prioritera originalbilden, men fall tillbaka på sparad sökväg/visad bild
      const candidates = [originalImageUrl, formData.job_image_url, jobImageDisplayUrl].filter(Boolean) as string[];
      for (const source of candidates) {
        const urlToEdit = await resolveEditableImageUrl(source);
        if (urlToEdit) {
  ...
  // Öppna editor för datorbild - samma logik som mobilbilden
  ...
  // Anställningsform är klar så snart en typ är vald — eventuella följdfält
  // (deltidsdagar/pass eller antal månader) har egna stjärnor.
  const employmentSectionComplete = !!formData.employment_type;
                      ...
                      {editingQuestion?.question_type === 'text' && (
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            className="bg-white/10 border-white/20 text-white placeholder:text-white h-11 !min-h-0 text-sm"
                          />
                        </div>
                      )}
                  ...
                  >
                    ...
                    {showCompanyTooltip && showApplicationForm && isScrolledTop && (
                      <>
                        ...
                        <div className="pointer-events-none absolute z-[999] top-[126px] md:top-[156px] -left-[98px] md:-left-[104px] flex items-center gap-1">
                    ...
                    <div className="relative w-full h-full rounded-[2rem] bg-black p-1 shadow-xl">
                      ...
                      <div className="relative w-full h-full rounded-[1.6rem] overflow-hidden bg-card-parium">
                        {/* Dynamic Island-style notch */}
                                ...
                                {showApplicationForm && (
                                  <div
                                    className="absolute inset-0 overflow-y-auto overflow-x-hidden bg-card-parium px-2 pb-2 pt-2 custom-scrollbar overscroll-contain"
                                  ...
                                  >
                                    <Button
                                      ...
                                      className="sticky top-0 ml-auto -mt-2 -mb-4 z-30 flex h-8 w-8 -translate-y-1 rounded-full text-white md:hover:text-white transition-all duration-200 [@media(hover:hover)]:hover:bg-black/40 [@media(hover:hover)]:hover:backdrop-blur-md [@media(hover:hover)]:hover:scale-110 active:scale-95"
                                    >
                                      <X className="h-4 w-4" />
                                    </Button>
                                  ...
                                  {showDesktopApplicationForm && (
                                    <div className="absolute inset-0 overflow-y-auto overflow-x-hidden bg-card-parium p-3 custom-scrollbar overscroll-contain">
                                      <Button
                                        ...
                                        className="sticky top-0 ml-auto -mb-10 z-30 flex h-9 w-9 rounded-full text-white md:hover:text-white transition-all duration-200 [@media(hover:hover)]:hover:bg-black/40 [@media(hover:hover)]:hover:backdrop-blur-md [@media(hover:hover)]:hover:scale-110 active:scale-95"
                                      >
                                        <X className="h-4 w-4" />
                                      </Button>
                                      <JobPostingPreviewContent
