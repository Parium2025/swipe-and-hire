...
const LandingForUsers = () => {
  ...
  const leftX = useTransform(scrollYProgress, [0.1, 0.35], [-80, 0]);
  const rightX = useTransform(scrollYProgress, [0.1, 0.35], [80, 0]);
  ...
  return (
    <section ref={sectionRef} className="relative py-24 sm:py-32 lg:py-40 px-5 sm:px-6 md:px-12 lg:px-24" aria-labelledby="for-users-heading">
      <div className="max-w-[1400px] mx-auto relative z-10">
        <motion.div className="mb-16 sm:mb-24" style={{ y: headerY, opacity: headerOpacity }}>
          ...
          <h2 id="for-users-heading" className="mt-4 text-3xl sm:text-5xl md:text-6xl font-black tracking-[-0.04em] text-white uppercase">
            En plattform.<span className="text-white/15"> Två perspektiv.</span>
          </h2>
        </motion.div>

        <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
          {/* Job seekers */}
          <motion.div style={{ x: leftX, opacity: cardsOpacity }}
            ...
            <div className="relative z-10">
              <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-secondary/[0.08] border border-secondary/15 text-secondary text-[10px] sm:text-xs font-semibold tracking-wider uppercase mb-8">Jobbsökare</span>
              <h3 className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-4 tracking-[-0.03em] uppercase">Hitta dröm<span className="text-white/20">jobbet</span></h3>
              <p className="text-white/35 text-sm leading-relaxed mb-8 max-w-md">Sluta scrolla genom oändliga listor. Swipea till jobb som faktiskt matchar dig.</p>
              <ul className="space-y-3 mb-10">
                {jobSeekerFeatures.map((f) => { const Icon = f.icon; return (
                  <li key={f.text} className="flex items-center gap-3 text-white/50 text-sm"><Icon className="w-4 h-4 text-secondary flex-shrink-0" strokeWidth={1.5} />{f.text}</li>
                ); })}
              </ul>
              <button onClick={() => goTo('job_seeker')} className="group/btn flex items-center gap-2 text-white font-semibold text-sm hover:text-secondary transition-colors">
                Kom igång <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
              </button>
            </div>
          </motion.div>

          {/* Employers */}
          <motion.div style={{ x: rightX, opacity: cardsOpacity }}
            ...
            <div className="absolute top-0 right-0 w-[300px] h-[300px] bg-secondary/[0.03] rounded-full blur-[80px] pointer-events-none group-hover:bg-secondary/[0.06] transition-colors duration-700" />
            <div className="relative z-10">
              <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-secondary/[0.08] border border-secondary/15 text-secondary text-[10px] sm:text-xs font-semibold tracking-wider uppercase mb-8">Arbetsgivare</span>
              <h3 className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-4 tracking-[-0.03em] uppercase">Hitta rätt<span className="text-white/20"> talang</span></h3>
              <p className="text-white/35 text-sm leading-relaxed mb-8 max-w-md">Sluta vänta veckor på ansökningar. Få AI-matchade kandidater direkt i fickan.</p>
              <ul className="space-y-3 mb-10">
