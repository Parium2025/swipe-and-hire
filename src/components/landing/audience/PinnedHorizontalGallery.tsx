...
const PinnedHorizontalGallery = () => {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // HÅRD SNAP gallery ↔ nästa sektion (samma känsla som 1↔2 och 2↔3).
  // När pin:en är ~klar och användaren gör en gest framåt → lås input och
  // GSAP-tween:a scrollTop till nästa sektion (offsetTop). Omvänt: när
  // användaren är vid nästa sektions topp och gör gest uppåt → tween:a
  // tillbaka till pin-end (sista kortet kvar synligt, INTE till början
  // av kortlistan — det skulle bryta känslan av kontinuitet).
  //
  // VIKTIGT: rör INTE warm/play/cache-event-bussen. Alla videos och
  // bilder preload:as fortfarande via samma mekanik som tidigare
  // (parium:gallery-warm → preload=auto, parium:gallery-enter → play).
  // ───────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    ...
    let cancelled = false;
    import('gsap').then((mod) => { if (!cancelled) gsapInstance = mod.default; });

    let snapping = false;
    let lockUntil = 0;
    let touchY: number | null = null;

    const getNext = () => section.nextElementSibling as HTMLElement | null;

    const pinEndY = () => Math.max(0, section.offsetTop + section.offsetHeight - root.clientHeight);

    const isAtPinEnd = () => {
      // Trigger när vi är inom de sista ~7% av pinen — ger marginal för
      // momentum-scroll utan att fyra för tidigt.
      const distance = Math.max(1, section.offsetHeight - root.clientHeight);
    ...
    const isAtNextStart = () => {
      ...
      // Top av nästa sektion inom första 30% av viewport = vi är "vid toppen".
      return rect.top >= -8 && rect.top < root.clientHeight * 0.30;
    };

    const snapTo = (targetY: number, duration = 0.82) => {
      if (snapping || !gsapInstance) return;
      snapping = true;
      lockUntil = performance.now() + duration * 1000 + 220;
    ...
    const onWheel = (e: WheelEvent) => {
      if (snapping || performance.now() < lockUntil) {
        e.preventDefault();
        e.stopPropagation();
        return;
      }
      const next = getNext();
      if (!next) return;
      if (e.deltaY > 4 && isAtPinEnd()) {
        e.preventDefault();
        e.stopPropagation();
        snapTo(next.offsetTop);
      } else if (e.deltaY < -4 && isAtNextStart()) {
        e.preventDefault();
        e.stopPropagation();
        snapTo(pinEndY());
      }
