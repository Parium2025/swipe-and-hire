...
const JobAdCard: React.FC<JobAdCardProps> = ({
  ...
  return (
    <section aria-label="Jobbannonskort" className="relative w-[160px] h-[320px] mx-auto">
      {/* Telefonram */}
      <div className="relative w-full h-full rounded-[1.4rem] bg-slate-950 p-1 shadow-2xl ring-1 ring-black/30">
        {/* Skärm */}
        <div className="relative w-full h-full rounded-[1.1rem] overflow-hidden bg-black">
          {/* Notch/status */}
          <div className="absolute top-1 left-1/2 -translate-x-1/2 z-20 h-0.5 w-8 rounded-full bg-black/60 border border-white/10"></div>
          ...
          <div className="absolute inset-x-0 bottom-0 p-3 text-white">
            ...
            <div className="mt-0.5 text-white/95 text-xs drop-shadow-[0_1px_3px_rgba(0,0,0,0.5)]">{company}</div>
            <div className="text-white/80 text-[10px] drop-shadow-[0_1px_3px_rgba(0,0,0,0.45)]">{location}</div>
          </div>
