...
const JobSeekerLayout = memo(({ children, developerView, onViewChange }: JobSeekerLayoutProps) => {
  ...
  if (isDesktop) {
    return (
      <>
        ...
        <div className="min-h-screen flex flex-col w-full relative" style={{ overflowX: 'clip' }}>
  ...
  return (
    <SidebarProvider defaultOpen={true}>
      {/* Fixed gradient background - covers viewport, own GPU layer */}
      <div className="fixed inset-0 bg-parium-gradient pointer-events-none z-0" style={{ transform: 'translateZ(0)' }} />
      
      {/* AnimatedBackground - fixed, own GPU layer, completely independent of scroll */}
      <AnimatedBackground showBubbles={false} />
      
      {/* Bubbles - FIXED position, own GPU layer, never participates in scroll compositing */}
      <div className="fixed left-0 right-0 top-14 pointer-events-none z-20" style={{ height: 'calc(100vh - 3.5rem)', contain: 'strict', transform: 'translateZ(0)' }}>
      ...
      <div className="min-h-screen flex flex-col w-full relative" style={{ overflowX: 'clip' }}>
        ...
        {/* Main content - document-level scroll (handled by compositor thread) */}
