...
const JobSeekerLayout = memo(({ children, overlay, developerView, onViewChange }: JobSeekerLayoutProps) => {
  ...
  return (
    <SidebarProvider defaultOpen={true} className="bg-gradient-parium">
      {/* Fixed gradient background - covers full viewport including safe areas */}
      <div
      ...
      <div className="h-[100dvh] flex w-full overflow-hidden relative" style={{ WebkitOverflowScrolling: 'touch' }}>
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative z-10 [padding-top:var(--top-chrome-content-offset,0px)]">
          ...
          <main data-main-scroll-container="true" className={`flex-1 min-h-0 overflow-x-hidden overflow-y-auto p-3 flex flex-col ${location.pathname === '/messages' ? 'pb-0 no-chrome-pad' : 'pb-8'}`} style={{ WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}>
            {children}
