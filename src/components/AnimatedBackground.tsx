...
export const AnimatedBackground = memo(({ showBubbles = true, hideRightBubbles = false, disableDesktopShift = false }: AnimatedBackgroundProps) => {
  return (
    <div className="fixed inset-0 pointer-events-none z-0">
      {showBubbles && (
        <>
          ...
          <div className={`absolute inset-0 transition-transform ${disableDesktopShift ? '' : 'lg:translate-x-[280px]'}`}>
            ...
            <div className="absolute top-24 left-20 w-3 h-3 bg-secondary/20 rounded-full animate-soft-bounce" style={{ animationDuration: '3s', animationDelay: '-2.1s', animationFillMode: 'backwards' }}></div>
      ...
      <div className="absolute -right-32 -bottom-32 w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 opacity-25 sm:opacity-30 md:opacity-40 lg:opacity-60 pointer-events-none pwa-bottom-glow">
