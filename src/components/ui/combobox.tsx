    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          aria-expanded={open}
          className={cn(
            "flex h-[var(--control-height)] w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-0 focus:border-white/40 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
            "bg-white/5 backdrop-blur-sm border-white/20 text-white hover:bg-white/10 hover:text-white hover:border-white/50",
            !value && "text-white",
            className
          )}
          disabled={disabled}
