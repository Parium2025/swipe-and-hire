...
export function useCandidateSummary({ applicantId, jobId, applicationId, cvUrl, open }: UseCandidateSummaryOptions) {
  ...
  const activeCacheKey = applicantId ? buildCacheKey(applicantId, jobId, cvUrl) : null;
  const syncedCacheKeyRef = useRef<string | null>(activeCacheKey);
  ...
  // ─── Sync state before paint when candidate/job/cv changes ─────
  useLayoutEffect(() => {
    if (!activeCacheKey) {
      syncedCacheKeyRef.current = null;
      setLoadingSummary(false);
      setGeneratingSummary(false);
      setAiSummary(null);
      return;
    ...
    if (syncedCacheKeyRef.current === activeCacheKey) return;

    syncedCacheKeyRef.current = activeCacheKey;
    setLoadingSummary(false);
    setGeneratingSummary(false);

    const cached = summaryCache.get(activeCacheKey)
      || getPersistedCacheValue<CandidateSummaryCacheValue>(SUMMARY_STORAGE_KEY, activeCacheKey);

    if (cached) {
      summaryCache.set(activeCacheKey, cached);
      setAiSummary(cached);
      return;
    }
  ...
  useEffect(() => {
    if (!applicantId || !user || !open || !activeCacheKey) return;
    if (aiSummary) return;

    (async () => {
      const result = await fetchAiSummary();
      if (result?.shouldAutoGenerate && cvUrl && !autoGeneratedKeysRef.current.has(activeCacheKey)) {
        autoGeneratedKeysRef.current.add(activeCacheKey);
        generateAiSummary(true);
      }
    })();
  }, [open, applicantId, user?.id, aiSummary, cvUrl, activeCacheKey, fetchAiSummary, generateAiSummary]);
