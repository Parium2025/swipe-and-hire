...
const h = vi.hoisted(() => ({
  ...
  /** Kö av saved_jobs-svar i anropsordning (deferred = hänger). */
  savedQueue: [] as Array<Deferred<number> | number>,
}));
    ...
    from: (table: string) => {
      ...
      if (table === 'saved_jobs') {
        return countQuery((id) => {
          const queued = h.savedQueue.shift();
          if (typeof queued === 'number') return Promise.resolve(queued);
...
describe('AuthProvider — samma ägare, ny accepterad session', () => {
  beforeEach(() => {
    localStorage.clear();
  ...
  it('samma konto får ny session (utan mellanliggande utloggning): gammalt svar får inte skriva', async () => {
    // Första refreshens saved_jobs-svar hänger.
    const stale = makeDeferred<number>();
    h.savedQueue.push(stale, 5, 5, 5);
    h.appCounts['jobseeker-1'] = 3;
    h.unreadCounts['jobseeker-1'] = 2;

    renderAuth(newClient());
    await waitFor(() => expect(h.authCallbacks.length).toBeGreaterThan(0), { timeout: 5000 });
    await settle(150);

    // Ny accepterad auth-händelse för SAMMA konto (ny session/token, inget null emellan).
    await act(async () => {
      h.authCallbacks.forEach((cb) =>
        cb('SIGNED_IN', {
          user: { id: 'jobseeker-1', email: 'jobseeker-1@example.com' },
    ...
    await waitFor(() => expect(sessionStorage.getItem(SAVED_KEY)).toBe('5'), { timeout: 5000 });

    // Det gamla svaret landar nu — samma user-id men äldre generation.
    await act(async () => {
      stale.resolve(99);
      await new Promise((r) => setTimeout(r, 300));
    });

    expect(sessionStorage.getItem(SAVED_KEY)).toBe('5');
  });
});
