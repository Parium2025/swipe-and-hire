...
describe('AuthProvider — kontobunden roll och refresh-auktoritet', () => {
  beforeEach(() => {
    localStorage.clear();
  ...
  it('P1-2: in-flight A-refresh skriver inte A:s kontosiffror efter byte till B', async () => {
    ...
    const deferredA = makeDeferred<number>();
    h.deferredSaved.set('jobseeker-1', deferredA);
    ...
    // Byt till konto B medan A:s refresh fortfarande väntar på svar
    cleanup();
    h.channelNames.length = 0;
    h.registrations.length = 0;
    h.userId = 'jobseeker-2';
    h.savedCounts['jobseeker-2'] = 7;
    h.appCounts['jobseeker-2'] = 5;
    h.unreadCounts['jobseeker-2'] = 3;

    const clientB = newClient();
    renderAuth(clientB);
    await waitFor(() => expect(sessionStorage.getItem(SAVED_KEY)).toBe('7'), { timeout: 5000 });
    const spyB = vi.spyOn(clientB, 'invalidateQueries');

    // A:s svar landar nu — det får inte påverka B:s vy
    await act(async () => {
      deferredA.resolve(99);
      await new Promise((r) => setTimeout(r, 300));
    });

    expect(sessionStorage.getItem(SAVED_KEY)).toBe('7');
    expect(sessionStorage.getItem(APPS_KEY)).toBe('5');
    expect(sessionStorage.getItem(UNREAD_KEY)).toBe('3');
    expect(spyB).not.toHaveBeenCalled();
  });

  it('P1-3: aktuell händelse ger exakt EN exact-scopad dashboard-invalidering', async () => {
    ...
    expect(statsCalls[0][0]).toEqual({
      queryKey: ['jobseeker-dashboard-stats', 'jobseeker-1'],
      exact: true,
    });
  });

  it('cleanup före debounce-timern hinner löpa ut ger noll invalidationer', async () => {
    const client = newClient();
    renderAuth(client);
    await waitFor(() => expect(h.channelNames).toContain('auth-saved-jobs-jobseeker-1'), {
      timeout: 5000,
