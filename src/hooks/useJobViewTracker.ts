...
/**
 * Multi-signal device detection combining:
 * 1. Touch capability (navigator.maxTouchPoints)
 * 2. Screen width
 * 3. User-Agent hints (tablet keywords)
 * 4. Pointer coarseness (CSS media query via matchMedia)
 */
function detectDeviceType(): 'mobile' | 'tablet' | 'desktop' {
  ...
  // Known tablet UA keywords (iPad, Android tablet, etc.)
  ...
  // Desktop: no touch OR fine pointer AND wide screen
  if (!hasTouchScreen && !hasCoarsePointer && width >= 1024) {
    return 'desktop';
  }

  // Tablet: touch device with wider screen OR known tablet UA
  if (isTabletUA || isIPadOS || (hasTouchScreen && width >= 768 && width < 1366)) {
    return 'tablet';
  }

  // Mobile: touch device with narrow screen
  if (hasTouchScreen || hasCoarsePointer || width < 768) {
    return 'mobile';
  }
...
export function useJobViewTracker({
  ...
  const recordView = useCallback(async () => {
    ...
    try {
      // Detect device type with multi-signal approach
