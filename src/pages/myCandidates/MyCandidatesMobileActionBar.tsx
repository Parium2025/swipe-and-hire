    ...
    <TooltipProvider delayDuration={300}>
      <div className="animate-in slide-in-from-bottom-4 duration-300 flex justify-center mt-2">
        <div className="flex items-center gap-2.5 bg-card-parium/95 backdrop-blur-md border border-white/20 rounded-full px-5 py-3 shadow-xl overflow-hidden min-w-0 max-w-full">
          ...
          >
            {allVisibleSelected ? <Square className="h-4.5 w-4.5 mr-2" /> : <CheckSquare className="h-4.5 w-4.5 mr-2" />}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                ...
                onMouseDown={(e) => e.preventDefault()}
                className={`flex items-center px-3 h-11 text-sm font-medium whitespace-nowrap flex-shrink-0 outline-none focus:outline-none transition-all duration-200 rounded-md active:scale-[0.97] touch-manipulation ${
                  selectedCount === 0 ? 'text-white/30 cursor-not-allowed' : 'text-white'
                }`}
              >
                <ArrowDown className="h-4.5 w-4.5 mr-2" />
                Flytta
              </button>
            </DropdownMenuTrigger>
