...
const FixedPhoneLayer = () => {
  ...
  const calculatePhoneMetrics = () => {
    ...
    const topSafeGap = tablet ? clamp(height * 0.065, 50, 82) : clamp(height * 0.075, 50, 76);
    const bottomSafe = tablet ? clamp(height * 0.075, 56, 88) : clamp(height * 0.09, 62, 88);
    const canvasTopBreathingRoom = tablet ? clamp(height * 0.065, 46, 76) : clamp(height * 0.09, 64, 92);
    const availableHeight = Math.max(180, height - textBottom - topSafeGap - bottomSafe);
    const maxCanvasHeight = Math.max(220, height - gap - bottomSafe);
    const finalHeight = Math.min(availableHeight, maxCanvasHeight, tablet ? clamp(height * 0.52, 360, 540) : clamp(height * 0.5, 260, 430));
    const targetVisualHeight = clamp(finalHeight - canvasTopBreathingRoom * 0.72, width <= 380 ? 180 : 196, tablet ? 374 : 323);
    const visualHeight = finalHeight;
