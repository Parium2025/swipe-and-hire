...
// ─────────────────────────────────────────────────────────────────
// Matchar en enskild token mot ett dokument-fält (redan normaliserat).
// Returnerar score 0–100 (0 = ingen träff).
// ─────────────────────────────────────────────────────────────────
const matchToken = (token: string, haystack: string): number => {
  if (!token) return 0;
  if (haystack.includes(token)) {
    // Stark bonus om hela ord börjar med token
    const words = haystack.split(/[\s,\-/]+/);
    if (words.some((w) => w === token)) return 100;
  ...
  for (const syn of synonyms) {
    ...
    if (haystack.includes(syn)) return 70;
  }
  ...
  if (token.length >= 4) {
    ...
    for (const w of words) {
      ...
      if (d <= maxDist) {
        const s = 60 - d * 15; // 60 vid d=0 (redan fångat ovan), 45/30 vid d=1/2
        if (s > best) best = s;
      }
    }
...
export const smartMatchScore = (query: string, fields: string[]): number => {
  ...
  for (const token of tokens) {
    ...
    if (s === 0) return 0; // Alla tokens måste matcha (AND-logik)
    total += s;
  }
  // Snitt så att längre queries inte automatiskt får högre score
