...
Deno.serve(async (req) => {
  ...
  const connections: Record<string, { connected: boolean; email: string | null; reconnectRequired?: boolean }> = {};
  for (const connectorId of SUPPORTED_CONNECTORS) {
    try {
      ...
      connections[connectorId] = connection
        ? { connected: true, email: connection.providerAccountEmail }
        : { connected: false, email: null };
    } catch (error) {
      console.error(`Kopplingsstatus misslyckades för ${connectorId}:`, error);
      connections[connectorId] = { connected: false, email: null, reconnectRequired: true };
    }
  }
