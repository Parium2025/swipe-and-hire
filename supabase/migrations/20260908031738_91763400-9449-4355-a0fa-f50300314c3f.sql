CREATE OR REPLACE FUNCTION public.get_conversation_summaries(
  p_user_id uuid,
  p_conversation_ids uuid[] DEFAULT NULL
)
RETURNS TABLE(
  conversation_id uuid,
  last_message_content text,
  last_message_sender_id uuid,
  last_message_created_at timestamp with time zone,
  last_message_is_system boolean,
  last_message_sender_identity text,
  unread_count bigint,
  counterpart_person_sender_id uuid
)
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  IF auth.uid() IS NULL OR auth.uid() <> p_user_id THEN
    RETURN;
  END IF;

  RETURN QUERY
  WITH my_memberships AS (
    SELECT cm.conversation_id, cm.last_read_at, cm.manually_unread
    FROM conversation_members cm
    WHERE cm.user_id = p_user_id
      AND (p_conversation_ids IS NULL OR cm.conversation_id = ANY(p_conversation_ids))
  ),
  latest_messages AS (
    SELECT DISTINCT ON (msg.conversation_id)
      msg.conversation_id, msg.content, msg.sender_id, msg.created_at,
      msg.is_system_message, msg.sender_identity
    FROM conversation_messages msg
    WHERE msg.conversation_id IN (SELECT mm.conversation_id FROM my_memberships mm)
    ORDER BY msg.conversation_id, msg.created_at DESC
  ),
  latest_counterpart AS (
    SELECT DISTINCT ON (msg.conversation_id)
      msg.conversation_id, msg.sender_id, msg.sender_identity, msg.is_system_message
    FROM conversation_messages msg
    WHERE msg.conversation_id IN (SELECT mm.conversation_id FROM my_memberships mm)
      AND msg.sender_id <> p_user_id
    ORDER BY msg.conversation_id, msg.created_at DESC
  ),
  unread AS (
    SELECT msg.conversation_id, COUNT(*)::bigint AS cnt
    FROM conversation_messages msg
    JOIN my_memberships mm ON mm.conversation_id = msg.conversation_id
    WHERE msg.sender_id <> p_user_id
      AND (mm.last_read_at IS NULL OR msg.created_at > mm.last_read_at)
    GROUP BY msg.conversation_id
  )
  SELECT
    mm.conversation_id,
    lm.content,
    lm.sender_id,
    lm.created_at,
    lm.is_system_message,
    lm.sender_identity,
    GREATEST(
      COALESCE(u.cnt, 0),
      CASE WHEN mm.manually_unread AND lm.conversation_id IS NOT NULL THEN 1 ELSE 0 END
    )::bigint,
    CASE
      WHEN lc.sender_identity = 'person' AND COALESCE(lc.is_system_message, false) = false
        THEN lc.sender_id
      ELSE NULL
    END
  FROM my_memberships mm
  LEFT JOIN latest_messages lm ON lm.conversation_id = mm.conversation_id
  LEFT JOIN latest_counterpart lc ON lc.conversation_id = mm.conversation_id
  LEFT JOIN unread u ON u.conversation_id = mm.conversation_id;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.get_conversation_summaries(uuid, uuid[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_conversation_summaries(uuid, uuid[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_conversation_summaries(uuid, uuid[]) TO service_role;