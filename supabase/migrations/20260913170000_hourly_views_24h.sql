...
BEGIN
  ...
  filtered_interviews AS (
    SELECT i.job_id,
           (i.status IN ('pending','confirmed','completed')
             AND (i.scheduled_at + make_interval(mins => COALESCE(i.duration_minutes, 30))) <= now()) AS is_done
    ...
    WHERE i.employer_id IN (SELECT user_id FROM org_members)
      ...
      AND (i.applicant_id IS NULL OR i.applicant_id NOT IN (SELECT user_id FROM org_members))
  ),
  prev_interviews AS (
    SELECT count(*)::int AS cnt,
           count(*) FILTER (
             WHERE i.status IN ('pending','confirmed','completed')
               AND (i.scheduled_at + make_interval(mins => COALESCE(i.duration_minutes, 30))) <= now()
           )::int AS done_cnt
    FROM public.interviews i
    WHERE i.employer_id IN (SELECT user_id FROM org_members)
  ...
  interview_counts AS (
    SELECT fi.job_id,
           ...
           count(*) FILTER (WHERE fi.is_done)::int AS interviews_completed_count
  ...
  job_stats AS (
    SELECT
      ...
      COALESCE(ic.interviews_completed_count, 0) AS interviews_completed_count
    FROM public.job_postings jp
  ...
  daily_stats AS (
    -- 24h-vyn grupperar per timme (svensk tid); ovriga perioder per dag.
    SELECT CASE
             WHEN p_days_back = 1
             THEN to_char(date_trunc('hour', viewed_at AT TIME ZONE 'Europe/Stockholm'), 'YYYY-MM-DD HH24:00')
             ELSE to_char(view_date, 'YYYY-MM-DD')
           END AS bucket,
           count(*)::int AS cnt
    FROM filtered_views
    GROUP BY 1
  ...
  trend_data AS (
    SELECT json_build_object(
      ...
      'current_interviews_completed', (SELECT count(*) FILTER (WHERE is_done)::int FROM filtered_interviews),
      'prev_interviews_completed', (SELECT done_cnt FROM prev_interviews)
    ) AS data
  )
  SELECT json_build_object(
    'jobs', COALESCE((
      SELECT json_agg(
        json_build_object(
          ...
          'interviews_completed_count', js.interviews_completed_count,
    ...
    'daily_views', COALESCE((
      SELECT json_agg(json_build_object('date', dss.bucket, 'count', dss.cnt) ORDER BY dss.bucket)
...
REVOKE ALL ON FUNCTION public.get_employer_analytics_v2(uuid, integer) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.get_employer_analytics_v2(uuid, integer) TO authenticated;