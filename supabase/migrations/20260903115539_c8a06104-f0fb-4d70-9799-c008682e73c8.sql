CREATE INDEX IF NOT EXISTS idx_job_postings_workplace_city_trgm
  ON public.job_postings USING gin (workplace_city gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_job_postings_workplace_municipality_trgm
  ON public.job_postings USING gin (workplace_municipality gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_job_postings_workplace_county_trgm
  ON public.job_postings USING gin (workplace_county gin_trgm_ops);

CREATE OR REPLACE FUNCTION public.count_search_jobs(p_search_query text DEFAULT NULL::text, p_city text DEFAULT NULL::text, p_county text DEFAULT NULL::text, p_employment_types text[] DEFAULT NULL::text[], p_category text DEFAULT NULL::text, p_salary_min integer DEFAULT NULL::integer, p_salary_max integer DEFAULT NULL::integer, p_employer_ids uuid[] DEFAULT NULL::uuid[], p_created_after timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS integer
 LANGUAGE plpgsql
 STABLE
 SECURITY DEFINER
...
DECLARE
  ...
  v_clean_query text;
  v_count integer;
  -- Skalningstak: över denna gräns slutar vi räkna i stället för att skanna hela tabellen.
...
CREATE OR REPLACE FUNCTION public.search_jobs(p_search_query text DEFAULT NULL::text, p_city text DEFAULT NULL::text, p_county text DEFAULT NULL::text, p_employment_types text[] DEFAULT NULL::text[], p_category text DEFAULT NULL::text, p_salary_min integer DEFAULT NULL::integer, p_salary_max integer DEFAULT NULL::integer, p_limit integer DEFAULT 100, p_offset integer DEFAULT 0, p_cursor_created_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_employer_ids uuid[] DEFAULT NULL::uuid[], p_created_after timestamp with time zone DEFAULT NULL::timestamp with time zone, p_sort text DEFAULT 'newest'::text, p_cursor_id uuid DEFAULT NULL::uuid, p_cursor_rank real DEFAULT NULL::real, p_cursor_views integer DEFAULT NULL::integer)
