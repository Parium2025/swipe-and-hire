...
ALTER TABLE public.job_postings ADD COLUMN IF NOT EXISTS part_time_shifts text[];
ALTER TABLE public.job_templates ADD COLUMN IF NOT EXISTS part_time_shifts text[];
...
RETURNS TABLE(
  ...
  image_updated_at timestamp with time zone, expires_at timestamp with time zone,
  search_rank real, image_focus_position text,
  duration_amount integer, duration_unit text, part_time_days text[], part_time_shifts text[]
)
LANGUAGE plpgsql
SECURITY DEFINER
