...
-- Normaliserar text: gemener, å/ä/ö/é/ü -> a/a/o/e/u, bort med skräptecken
CREATE OR REPLACE FUNCTION public.parium_norm(t text)
RETURNS text
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
...
AS $$
  SELECT btrim(regexp_replace(
    translate(lower(coalesce(t, '')),
      'åäöáàâãéèêëíìîïóòôõúùûüýÿñçøæ',
      'aaoaaaaeeeeiiiiooooouuuuyyncoa'),
    '[^a-z0-9 ]', ' ', 'g'));
$$;

CREATE INDEX IF NOT EXISTS job_applications_name_trgm_idx
  ON public.job_applications
  USING gin (public.parium_norm(coalesce(first_name,'') || ' ' || coalesce(last_name,'')) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS job_applications_name_rev_trgm_idx
  ON public.job_applications
  USING gin (public.parium_norm(coalesce(last_name,'') || ' ' || coalesce(first_name,'')) gin_trgm_ops);
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE (p_status IS NULL OR a.status = p_status)
      AND (
        v_search IS NULL
        -- 1) Exakt/prefix via fulltext (snabbt, indexerat)
        OR (v_tsquery IS NOT NULL AND a.search_vector @@ to_tsquery('simple', v_tsquery))
        -- 2) Trigram på namn i båda ordningarna: tål stavfel OCH omvänd ordning
        ...
        -- 3) Alla sökord måste finnas någonstans (delsträng eller fuzzy ord-likhet)
        OR (
          ...
          AND NOT EXISTS (
            ...
            WHERE NOT (
              ...
              OR word_similarity(tok, public.parium_norm(
                coalesce(a.first_name,'') || ' ' || coalesce(a.last_name,'') || ' ' ||
                coalesce(a.location,'') || ' ' || coalesce(j.title,'')
              )) > 0.55
            )
