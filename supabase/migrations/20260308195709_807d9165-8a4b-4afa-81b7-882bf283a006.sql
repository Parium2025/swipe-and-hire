...
-- 1. Trigger to decrement applications_count when an application is deleted
...
CREATE TRIGGER trg_decrement_applications_count
  AFTER DELETE ON public.job_applications
  FOR EACH ROW
  EXECUTE FUNCTION public.decrement_job_applications_count();

-- 2. Update analytics RPC to filter out cancelled interviews
...
BEGIN
  ...
  hired_counts AS (
    SELECT ja.job_id, COUNT(*)::int as cnt
    FROM job_applications ja
    WHERE ja.job_id IN (SELECT oj.id FROM org_jobs oj)
      AND ja.status = 'hired'
    GROUP BY ja.job_id
  )
  SELECT json_build_object(
    'jobs', COALESCE((
      ...
      LEFT JOIN hired_counts hc ON hc.job_id = oj.id
    ), '[]'::json)
  ) INTO v_result;
