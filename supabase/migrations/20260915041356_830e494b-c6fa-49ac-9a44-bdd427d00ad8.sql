-- 1) Tillåt kollegor i samma organisation att söka i en kollegas kandidatlista
CREATE OR REPLACE FUNCTION public.search_my_candidates(p_recruiter_id uuid, p_search_query text, p_limit integer DEFAULT 50, p_cursor_updated_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_cursor_id uuid DEFAULT NULL::uuid, p_list_id uuid DEFAULT NULL::uuid, p_stage text DEFAULT NULL::text)
...
BEGIN
  ...
  IF auth.uid() <> p_recruiter_id AND NOT public.same_organization(auth.uid(), p_recruiter_id) THEN
    RETURN;
  END IF;
...
-- 2) Antal per steg: kan nu räknas även för en kollegas lista (RLS styr åtkomsten)
DROP FUNCTION IF EXISTS public.count_my_candidates_per_stage(uuid);

CREATE OR REPLACE FUNCTION public.count_my_candidates_per_stage(p_list_id uuid DEFAULT NULL::uuid, p_recruiter_id uuid DEFAULT NULL::uuid)
...
AS $function$
  ...
  FROM public.my_candidates mc
  WHERE mc.recruiter_id = COALESCE(p_recruiter_id, auth.uid())
    AND (p_list_id IS NULL OR mc.list_id = p_list_id)
  GROUP BY mc.stage
$function$;