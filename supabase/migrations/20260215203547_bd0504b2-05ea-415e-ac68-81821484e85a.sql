...
-- Fix register_session: if token already exists, just bump heartbeat (same-device tabs)
...
BEGIN
  ...
  -- If this token already exists for this user, just bump heartbeat and return.
  -- This handles multiple tabs on the same browser sharing the same localStorage token.
  ...
  -- New token — clean up stale sessions first
