...
-- 1. Sökindex för CV-text
CREATE INDEX IF NOT EXISTS idx_cv_summaries_text_trgm
  ON public.profile_cv_summaries
  USING gin ((lower(coalesce(raw_text,'') || ' ' || coalesce(summary_text,''))) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_cv_summaries_user ON public.profile_cv_summaries (user_id);

-- 2. Sökindex för anteckningar
CREATE INDEX IF NOT EXISTS idx_candidate_notes_text_trgm
  ON public.candidate_notes USING gin (lower(note) gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_candidate_notes_applicant ON public.candidate_notes (applicant_id);

-- 3. Index för markörpaginering
...
RETURNS TABLE(
  id uuid, job_id uuid, applicant_id uuid, first_name text, last_name text, email text,
  phone text, location text, bio text, cv_url text, age integer, employment_status text,
  work_schedule text, availability text, custom_answers jsonb, questions_snapshot jsonb,
  status text, applied_at timestamp with time zone, updated_at timestamp with time zone,
  viewed_at timestamp with time zone, job_title text, job_occupation text, rating integer,
  total_count bigint, match_source text, account_deleted boolean
)
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Markörpaginering används bara för tidsbaserade sorteringar (default-vyn).
  ...
  base AS (
    ...
    WHERE (p_status IS NULL OR a.status = p_status)
      ...
      AND (
        ...
        -- Nytt: träff i CV-text
        OR EXISTS (
          SELECT 1 FROM public.profile_cv_summaries cs
          WHERE cs.user_id = a.applicant_id
            AND lower(coalesce(cs.raw_text,'') || ' ' || coalesce(cs.summary_text,'')) LIKE '%' || lower(v_search) || '%'
        )
        -- Nytt: träff i egna/organisationens anteckningar
        OR EXISTS (
          SELECT 1 FROM public.candidate_notes cn
          WHERE cn.applicant_id = a.applicant_id
            AND lower(cn.note) LIKE '%' || lower(v_search) || '%'
  ...
  SELECT
    ...
    s.match_source,
    NOT EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = s.applicant_id) AS account_deleted
  FROM scored s
  ORDER BY
...
GRANT EXECUTE ON FUNCTION public.search_employer_candidates(text, jsonb, text, text, integer, integer, boolean, timestamptz, uuid, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.search_employer_candidates(text, jsonb, text, text, integer, integer, boolean, timestamptz, uuid, integer) TO service_role;
