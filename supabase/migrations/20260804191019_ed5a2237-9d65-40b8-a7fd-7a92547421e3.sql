...
BEGIN
  ...
  v_env := COALESCE(NULLIF(_environment, ''), 'production');
  ...
  LOOP
    IF NOT EXISTS (
      ...
      VALUES (v_admin_id, 'system_app_failure',
        LEFT(COALESCE(NULLIF(_title, ''), 'Appfel upptäckt'), 180), v_alert_body, v_alert_metadata);
...
DELETE FROM public.notifications WHERE type = 'system_app_failure';