...
BEGIN
  ...
  dropoff AS (
    ...
    ORDER BY 
      CASE WHEN COALESCE(jp.applications_count, 0) = 0 THEN 0
           ELSE (COALESCE(jp.applications_count, 0)::float / GREATEST(COALESCE(jp.views_count, 0), 1))
      END ASC,
      COALESCE(jp.views_count, 0) DESC
  )
