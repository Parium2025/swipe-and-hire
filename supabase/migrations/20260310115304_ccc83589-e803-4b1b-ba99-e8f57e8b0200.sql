-- Drop the old 2-parameter version of record_job_view to avoid ambiguity
DROP FUNCTION IF EXISTS public.record_job_view(uuid, uuid);

-- Update get_employer_analytics_v2 to exclude the employer's own views
-- and include expires_at in TTFA data
CREATE OR REPLACE FUNCTION public.get_employer_analytics_v2(p_user_id uuid, p_days_back int DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF auth.uid() IS NULL OR auth.uid() <> p_user_id THEN
    RETURN '{"jobs":[],"device_breakdown":[],"daily_views":[],"trends":null,"best_day":null,"time_to_first_application":[]}'::json;
  END IF;
  ...
  -- Exclude views from any org member (employer's own views)
  filtered_views AS (
    SELECT jv.job_id, jv.device_type, jv.viewed_at::date as view_date, jv.viewed_at
    FROM job_views jv
