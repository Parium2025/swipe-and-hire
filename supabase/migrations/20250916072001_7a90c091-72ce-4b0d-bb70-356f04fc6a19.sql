...
DROP FUNCTION public.get_consented_profile_for_employer(uuid);

-- Recreate the function with last_name included
...
BEGIN
  ...
  -- Return consented profile data including both first and last name
  RETURN QUERY 
  SELECT 
    p.id,
    p.user_id,
