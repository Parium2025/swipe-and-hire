    ...
    'new_applications', (
      ...
      WHERE ja.job_id IN (SELECT jid FROM allowed)
        ...
        AND NOT EXISTS (
          ...
          WHERE v.application_id = ja.id AND v.viewer_id = p_user_id
        )
    ),
    'saved_favorites', (
...
REVOKE ALL ON FUNCTION public.get_employer_inbox_stats(uuid, uuid[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_employer_inbox_stats(uuid, uuid[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_employer_inbox_stats(uuid, uuid[]) TO service_role;